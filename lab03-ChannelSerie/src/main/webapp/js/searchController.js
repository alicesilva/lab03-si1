...
app.controller("SearchController", function($scope, $rootScope, $http, $location){
	...
	$scope.addSerieToProfile = function(name, year, image){
		...
		token = localStorage.getItem("userToken")
		$http.defaults.headers.common.Authorization = 'Bearer ' + token;
		$http.post("/admin/registerSerieToProfile/"+$rootScope.userLoggedIn.id, $scope.serieProfile)
	...
	$scope.addSerieToWatchlist = function(name, year, image){
		...
		token = localStorage.getItem("userToken")
		$http.defaults.headers.common.Authorization = 'Bearer ' + token;
		$http.post("/admin/registerSerieToWatchlist/"+$rootScope.userLoggedIn.id, $scope.serieWatchlist)
		.then(function(response){
			if(response.data == ""){
				alert("Series has already been added to the profile or watch list, it is impossible to add it again");
			}else{
